...
5. **设计对齐** — 取数逻辑/字段映射与设计方案核对；SELECT 字段与目标表 DDL 的字段名/类型/分区对齐
6. **结构与可读性** — 嵌套深度（>2 层子查询应拆 TMP）、同一表达式是否重复内联、业务清单是否硬编码、汇总行是否混入聚合、防御逻辑是否注释了对应边界事故

---

## 输出格式（解析契约，必须严格遵守）

生成 Markdown 报告。**发现的问题必须用列表行格式**（管道按 `- [级别] ` 前缀解析，表格格式无法被读取）：
...
🚫 **禁止放过嵌套超过 2 层的子查询** —— 超过时应该用 TMP 临时表拆分  
...
9. **检查设计对齐**：取数逻辑与设计方案一致？SELECT 字段与 DDL 字段名/类型/分区对齐？
...
6. **设计方案/表结构为「未获取」** → 跳过设计对齐核对项，不虚构字段
...
| 级别 | 定义 | 示例 |
|------|------|------|
| **Critical** | 代码层**可确定修复**的缺陷（修改 SQL 即可消除） | 缺少分区过滤、主键重复、逻辑矛盾、除法无 NULLIF |
| **Warning** | 不影响正确性的质量问题 | SELECT *、嵌套过深、缺少注释 |
| **Confirm** | 需**人工确认**的口径或依赖问题（改代码无法消除） | 源表缺字段、状态码字典未知、表名冲突待裁决、口径需业务拍板 |
...
- [Critical] (WHERE 子句) source_table 缺少分区过滤 — 添加 `where inc_day = '${bizdate}'`
- [Warning] (SELECT 子句) 使用了 SELECT * — 列出需要的字段
- [Warning] (文件头) 缺少文件头注释 — 添加需求、目标表、作者等元数据
