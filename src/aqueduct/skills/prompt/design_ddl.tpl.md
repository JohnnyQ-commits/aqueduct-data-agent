...
**产出范围硬约束**：先从需求文档中清点产出表清单（需求点名要交付的每一张表），DDL 必须逐表建齐——目标产出层（如 ads_/dws_ 层）与中间层（tmp_）各自都要有对应的 CREATE TABLE，一张都不能少。
...
🚫 **禁止遗漏主键字段** —— 必须明确主键定义  
🚫 **禁止只建 TMP 中间层而遗漏目标产出表** —— 需求点名交付的每一张表都必须有对应 CREATE TABLE；只建中间层不建产出层属未完成交付，将被审查判为 Critical
