...
- **FAIL 先看评分卡的"网关健康"列再判定质量退步**——评分卡自动从 task log 提取
  LLM 超时/重试/平台连接异常/降级计数，FAIL 且 LLM 网关异常时自动附 ⚠️ 污染警示。
  首跑实例：迭代用例 FAIL 的根因是 LLM 网关 ConnectionRefused（超时→重试→55 字符
  残响应被当正文→门禁降级落盘），SQL/linter/DQC 本体全净
- **迭代用例的试跑维度不作为质量信号**：其目标表/退款源表是 demo 虚构表，真实平台不存在
  （MCP 查不到），试跑按设计 skip 或报表不存在；试跑维度的有效信号来自全新用例
  （其两张源表在平台真实存在）
- 同一需求连跑 3 次的波动需先排除网关因素（看网关健康列）再判定退步
- 模板（`.tpl.md` / skill prompt）有 diff 的改动必须附评估报告，分数不降才可合并（阶段 2 守门）
- 评估进程用 nohup 脱离会话跑（首跑曾被会话退出杀死，损失 30 分钟）
