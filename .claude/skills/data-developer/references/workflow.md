...
1. 读取需求文档
2. 查询 MCP 工具获取源表结构
3. 匹配 knowledge/domains/ 业务域（若有 internal/knowledge 则优先）
4. 识别歧义点
5. 输出"需求理解摘要 + 问题清单"向用户确认
6. **用户确认后，将回答回写到 Phase1 文档对应问题下方**（引用块 `>` + ✅ 标记），形成闭环
**输出**: `output/{需求名}/Phase1-需求理解摘要.md`（含问题 + 回答 + 确认结论）
