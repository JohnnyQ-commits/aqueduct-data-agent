...
- `as 别名` 必须与表达式同行（表达式过长换行时，`as 别名` 跟在最后一行表达式尾部，禁止 `as` 单独成行做对齐）
- 禁止 `select d.*` / `select 表名.*` —— 派生表字段必须显式列出（列名即文档，防上游加列静默带脏）
- 示例: `case when d.area_code = '001' then a.x else d.x end as x,`（一行到底，不做右对齐缩进）
...
> 核心: 机器生成的 SQL 追求"一次算对", 资深手写追求"半年后自己能看懂、业务方敢签字、出问题 10 分钟能定位"。
> 来源: 2026-09 盘点需求 CR-001~003 多轮业务方代码评审实录沉淀。

### 12.1 嵌套深度红线: 超 2 层派生表就拆 tmp

- 同一个 INSERT 里派生表链超过 2 层, 一律拆出中间 tmp 表摊平
- 反例: cur→mid→win→outer 四层嵌套 + `sum() over ()` 单行窗口当聚合 (盘点 CR-003 前身, 业务方评"不像人手写")
- 正例: STEP 4a 明细毛坯 tmp(截零只算一遍) + STEP 4b 主 INSERT 摊平一层(001 行左联 28 地区汇总覆写)

### 12.2 同一表达式只写一遍

- 同一计算被 ≥2 个字段引用时, 先落中间列(或 tmp), 后续引用列名
- 反例: 截零表达式在 3 个字段里重复内联, 改口径漏改一处即数据事故
- SQL 同层不能引用自己的别名 ≠ 只能内联重复, 正确解法是加一层派生表或 tmp

### 12.3 不硬编码业务事实

- 业务清单(地区/网点类型/白名单/排除名单)写死 `not in (...)` → 业务一变静默出错
- 正确: 让数据自己说话——JOIN 维表/主表自然淘汰越界行, 范围变化零代码改动
- 反例: `not in ('898Y','991Y')` 硬编码排除名单(新地区接入须人肉维护, 漏了就静默错)
- 汇总行(001/全国行)口径集中在一个位置实现(建议最终层), 不在多个 tmp 分散

### 12.4 防御写在数据边界, 注释对应真实事故

- 防御点清单: 月末/期末 NULL、维表缺失、空分区、上游迟到、汇总行混入聚合
- 每个防御逻辑注释写明防的是什么事故; 不踩过的坑不过度防御
- 反例: `greatest(x, 0)` 会吞 NULL(效能缺失变"无缺口") → 正确: 判空截零 `case when x is null then null else greatest(x, 0) end`
- 反例: 汇总行混入 sum → 全分区聚合翻倍 (同: 电商流失 mi 表 001 行教训)

### 12.5 数据驱动替代日历推导(取最新分区/最新状态)

- 源表分区产出滞后于业务周期(周表当周分区周中才落地)时, 按日历公式猜分区必错
- 正确: `inc_day = (select max(inc_day) from t where inc_day <= '$[T-1]' and <粒度过滤>)`
- 子查询的过滤条件(如 dim_level='dept')用于跳过无目标粒度行的空分区
- 实录: 按日历算分区在周一~周三指向未产出分区 → 缺口字段全空 (09-20 事故)

### 12.6 聪明写法克制使用

- grouping sets / 窗口函数 / 递归 CTE 只在收益明确时用(数据量逼出性能需求、语义确实需要)
- 小表(<百行)用 `over ()` 全窗口当聚合 = 拿可读性换不存在的性能
- 能用 GROUP BY + JOIN 讲清楚的, 不用炫技

### 12.7 命名与注释面向业务

- 别名/注释/步骤标题用业务话术(如"剥离率目标""协防问题"), 业务方可对照口径文档逐行核对
- 每步头部注释三件套: 用途 + 口径依据(业务方谁哪天确认的) + 排查 SQL(预期行数)
- 别名禁止机器式命名(如 s_gap_new_area_latest), 变量名是给人看的文档

### 12.8 交付前自检: 读一遍像不像人写的

- 通读自己的 SQL, 任何一段让业务方/同事说"不像人手写"的, 都按本章重构
- 重构后必须重新验证: 端到端实测结果与重构前逐项一致才可提交
...
- [ ] **单 SELECT 链派生表 ≤2 层, 超过拆 tmp 摊平 (§12.1)**
- [ ] **同一计算表达式只出现一次, 复用靠中间列/tmp (§12.2)**
- [ ] **业务清单不硬编码 not in, 由维表/主表 JOIN 决定范围 (§12.3)**
- [ ] **汇总行(001)不参与往上汇总的聚合, 汇总口径集中在最终层 (§12.3/12.4)**
- [ ] **取最新分区用 max(inc_day) 数据驱动, 不按日历公式猜 (§12.5)**
- [ ] **通读像人写的, 重构后端到端实测与重构前一致 (§12.8)**
- [ ] 每个临时表有排查注释
- [ ] 有配套的执行流程文档
