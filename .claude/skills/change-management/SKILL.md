...
3. **新业务表述检索**：变更引入新指标/口径（业务表述）时，运行 `python src/aqueduct/memory/history.py "<业务表述>"` 检索全库历史命中——已有实现直接对齐其表.字段口径复用，禁止重复实现
4. 输出：变更字段清单 + 逻辑差异说明
...
| V8 编译实测（新片段） | 变更新增/改动的 SQL 片段是否通过编译验证——按数据验证 Checklist 第〇节能力分级：L2/L1 在可执行通道实测编译（命令示例见 data-developer skill《平台通道适配映射》第二节），L0 降级静态自检并逐段标注"未编译"；每个"编译通过"须附执行证据，无证据标 `⏳ 未实测` |
| 待确认问题 | 标记已确认/未确认状态 |
